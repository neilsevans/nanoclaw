...
export function startOllamaProxy(
  ...
  return new Promise((resolve, reject) => {
    const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
      ...
      req.on('end', async () => {
        try {
          ...
          const userMessages = claudeReq.messages.filter((m) => m.role === 'user');
          ...
          logger.info({ model: ollamaModel, promptLen: prompt.length }, 'Ollama proxy: forwarding request');
          ...
          const ollamaRes = await new Promise<string>((resolve, reject) => {
            ...
            const options = {
              ...
              headers: {
                ...
                'Content-Length': Buffer.byteLength(JSON.stringify(ollamaReq)),
              },
            };
          ...
          try {
            ...
            logger.error({ parseErr, ollamaRes: ollamaRes.slice(0, 200) }, 'Ollama proxy: failed to parse Ollama response');
          ...
          if (!responseText) {
            logger.warn({ ollamaData }, 'Ollama proxy: empty response from Ollama');
            res.writeHead(502, { 'Content-Type': 'application/json' });
          ...
          logger.info(
            { responseLen: responseText.length, duration: ollamaData.total_duration },
    ...
    server.listen(port, host, () => {
      logger.info({ port, host, ollamaHost, ollamaModel }, 'Ollama proxy started');
      resolve(server);
    });
